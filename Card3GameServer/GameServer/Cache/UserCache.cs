...
namespace GameServer.Cache
{
    ...
    public class UserCache
    {
        ...
        private void Create(MySqlConnection conn, string name, int accountID)//添加帐号 
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("insert into hero set name =@name , accountID=@accountID,been=@been", conn);
                cmd.Parameters.AddWithValue("name", name);
                cmd.Parameters.AddWithValue("accountID", accountID);
                //cmd.Parameters.AddWithValue("been",been);
                cmd.ExecuteNonQuery();
            }
            catch (Exception e)
            {
        ...
        private int GetId(MySqlConnection conn, int id)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("id", id);
                reader = cmd.ExecuteReader();
                //让 SqlDataReader 前进到下一条记录。如果存在更多行，则为 true；否则为 false。
                if (reader.Read())
                {
                    int accountID = reader.GetInt32("accountID");
                    return accountID;

...
namespace GameServer.Cache.Match
{
    ...
    public class MatchCache
    {
        ...
        /// </summary>
        public Dictionary<int, int> uidRoomIdDict = new Dictionary<int, int>();

        /// <summary>
        /// 代表  正在等待的  房间id  和 房间的数据模型 的映射
        /// </summary>
        public Dictionary<int, MatchRoom> idModelDict = new Dictionary<int, MatchRoom>();
        /// <summary>
        /// 储存创建的房间
        /// </summary>
        public List<MatchRoom> roomsiza = new List<MatchRoom>();
        /// <summary>
        /// 代表 重用的房间 队列
        /// </summary>
        ...
        /// <returns></returns>
        public MatchRoom establish(int userId, ClientPeer client)
        {
            //自己开房
            MatchRoom room = new MatchRoom(id.Add_Get());

            roomsiza.Add(room);

            room.Enter(userId, client);
            idModelDict.Add(room.Id, room);
            if (uidRoomIdDict.ContainsKey(userId))
            {
                return room;
            }
            else

...
namespace GameServer.Logic
{
    public class FightHandler : IHandler
    {
        ...
        private void leave(ClientPeer client)
        {
            SingleExecute.Instance.Execute(
                () =>
                {
                   ...
                   // MatchRoom room1 = matchCache.GetRoom(userId);
                    ...
                    if (room.LeaveUIdList.Count == 3)
                    {
                        ...
                        {
                            UserModel um = userCache.GetModelById(client, room.LeaveUIdList[i]);
                            um.RunCount++;
        ...
        private void gameOver( ClientPeer client, int userId, FightRoom room)
        {
            //获取获胜身份 所有的玩家id
            ...
            {
                UserModel um = userCache.GetModelById(client, winUIds[i]);
                um.WinCount++;
                um.Been += winBeen;
                um.Exp += 100;
            ...
            {
                UserModel um = userCache.GetModelById(client, loseUIds[i]);
                um.LoseCount++;
                um.Been -= winBeen;
                um.Exp += 10;
                int maxExp = um.Lv * 100;

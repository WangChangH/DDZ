...
namespace GameServer.Logic
{
    ...
    public class AccountHandler : IHandler
    {
        ...
        private void regist(ClientPeer client, string account, string password)
        {
            ...
            {
                if (accountCache.IsExist(client.MySqlConnection, account))
                {
                    //表示账号已经存在 
                ...
                //可以注册了
                accountCache.Create(client.MySqlConnection, account, password);
        ...
        private void login(ClientPeer client, string account, string password)
        {
            SingleExecute.Instance.Execute(() =>
            {
                ...
                if (!accountCache.IsMatch(client.MySqlConnection, password))
                {
                    //表示账号密码不匹配
